...
import { CurrentWeatherData, Forecast, ForecastData, ForecastListItem, Weather } from './types';
...
export class WeatherService {
  ...
  getTodaysWeather(city: string): Observable<Weather> {
    return this.http
  ...
  private handleError(error: Error | HttpErrorResponse): Observable<never> {
    if (error instanceof HttpErrorResponse) {
      ...
      if (!navigator.onLine) {
        ...
        return throwError('It seems like you are not connected to the internet');
      } else {
