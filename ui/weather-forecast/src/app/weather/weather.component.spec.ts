...
describe('WeatherComponent', () => {
  ...
  it('should not have weather data immediately after the component is constructed', () => {
    expect(component.weather).toBeFalsy();
    expect(component.forecast.length).not.toBeGreaterThan(0, 'No forecasts');
  });

  it('should display the current weather, five day forecast after the component initializes', () => {
    ...
    expect(component.city).toEqual(defaultCity, 'default city');
    fixture.detectChanges();
    const cardTitle = nativeEl.querySelector('mat-card-title') as HTMLElement;
    const cardSubtitles = nativeEl.querySelectorAll('mat-card-subtitle');
    const temp = nativeEl.querySelector('.large.temp') as HTMLElement;

    const forecastItems = nativeEl.querySelectorAll('mat-list-item.forecast');
    const weatherIcon = nativeEl.querySelector('.huge.my-wi') as HTMLElement;
  ...
  it('should show the current weather for the default city based on the weather', () => {
    const getWeatherSpy = spyOn(component, 'getWeather').and.callThrough();

    expect(component.city).toEqual('Pune', 'default city');
  ...
  it('should show the current weather and forecast for a valid city when it is typed into the search input', () => {
    spyOn(service, 'getTodaysWeather').and.callFake(() =>
  ...
  it('should throw an error when the city being searched for is invalid', () => {
    ...
    const getCurrentWeatherSpy = spyOn(service, 'getTodaysWeather');
    ...
    fixture.detectChanges();
    const err = nativeEl.querySelector('.err') as HTMLElement;
